...
function SetAutoload(configName)
	if writefile and AutoloadFile then
		...
		print("[OBS] Autoload config set to: " .. configName)
	end
end
...
function ClearAutoload()
	if delfile and AutoloadFile and isfile(AutoloadFile) then
		...
		print("[OBS] Autoload config cleared")
	end
end

function SaveConfigAs(configName)
	if not configName or configName == "" then
		warn("[OBS] Config name cannot be empty")
		return false
	end
	...
	if writefile and gameFolder then
		...
		print("[OBS] Config saved as: " .. configName)
		return true
	end
	return false
end
	...
	if isfile and isfile(filepath) then
		...
		if success and type(result) == "table" then
			if result._version == CURRENT_VERSION then
				...
				CurrentConfigName = configName
				LoadConfigElements()
				print("[OBS] Config loaded: " .. configName)
				return true
			else
				warn("[OBS] Config version mismatch")
				return false
			end
		else
			warn("[OBS] Failed to decode config")
			return false
		end
	else
		warn("[OBS] Config file not found: " .. configName)
		return false
	end
end
...
function DeleteConfig(configName)
	...
	if delfile and isfile(filepath) then
		delfile(filepath)
		print("[OBS] Config deleted: " .. configName)
...
function ResetElements()
	...
	print("[OBS] Elements reset to defaults")
end

function LoadExternalConfigFromJSON(jsonString)
	if not jsonString or jsonString == "" then
		warn("[OBS] External config JSON kosong")
		return false
	end
	...
	if not ok or type(data) ~= "table" then
		warn("[OBS] External config JSON invalid")
		return false
	end

	if data._version and CURRENT_VERSION and data._version ~= CURRENT_VERSION then
		warn("[OBS] External config version mismatch")
		return false
	end
	...
	print("[OBS] External config loaded from JSON input")
	return true
end
...
function Obsidia:MakeNotify(NotifyConfig)
	NotifyConfig = NotifyConfig or {}
	NotifyConfig.Title = NotifyConfig.Title or "Obsidia"
...
function obsidia(msg, delay, color, title, desc)
	return Obsidia:MakeNotify({
		Title = title or "Obsidia",
		Description = desc or "Notification",
		Content = msg or "Content",
		Color = color or Color3.fromRGB(127, 0, 255),
		Delay = delay or 4,
	})
end
...
local function ShowLoading()
	...
	stroke.Color = Color3.fromRGB(147, 0, 255)
	stroke.Parent = bg
	...
	title.TextSize = 15
	title.TextXAlignment = Enum.TextXAlignment.Left
	title.Text = "OBSIDIA ANNOUNCEMENT"
	title.TextColor3 = Color3.fromRGB(165, 50, 255)
	title.Parent = bg
	...
	msg.Text = "Obsidia Script are free script!"
	msg.TextColor3 = Color3.fromRGB(230, 210, 255)
	msg.Parent = bg
	...
	bar.BackgroundColor3 = Color3.fromRGB(147, 0, 255)
...
function Obsidia:Window(GuiConfig)
	if not _G.__OBS_LOADED then
		_G.__OBS_LOADED = true
		ShowLoading()
		task.wait()
	end

	GuiConfig = GuiConfig or {}
	GuiConfig.Title = GuiConfig.Title or "Obsidia"
	GuiConfig.Footer = GuiConfig.Footer or "Obsidia :3"
	GuiConfig.Color = GuiConfig.Color or Color3.fromRGB(127, 0, 255)
	GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 120
	GuiConfig.Version = GuiConfig.Version or 1

	local folderName = GuiConfig.Folder or GuiConfig.Title:gsub("[^%w_ ]", ""):gsub("%s+", "_")
	gameFolder = "Obsidia/" .. folderName
	...
	local Obsidiaex = Instance.new("ScreenGui")
	...
	Obsidiaex.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	Obsidiaex.Name = "Obsidia"
	Obsidiaex.ResetOnSpawn = false
	Obsidiaex.Parent = game:GetService("CoreGui")
	...
	DropShadowHolder.Parent = Obsidiaex

	DropShadowHolder.Position = UDim2.new(
		0,
		(Obsidiaex.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2),
		0,
		(Obsidiaex.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2)
	)
	...
	function GuiFunc:DestroyGui()
		if CoreGui:FindFirstChild("Obsidia") then
			Obsidiaex:Destroy()
		end
	end
	...
	Close.Activated:Connect(function()
		...
		Gradient.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.0, Color3.fromRGB(110, 0, 230)),
			ColorSequenceKeypoint.new(0.25, Color3.fromRGB(200, 100, 255)),
			ColorSequenceKeypoint.new(0.5, Color3.fromRGB(85, 0, 200)),
			ColorSequenceKeypoint.new(0.75, Color3.fromRGB(200, 100, 255)),
			ColorSequenceKeypoint.new(1.0, Color3.fromRGB(110, 0, 230)),
		})
	...
	DropdownSelect.BackgroundColor3 = Color3.fromRGB(45, 0, 110)
	...
	UIStroke14.Color = Color3.fromRGB(127, 0, 255)
	UIStroke14.Thickness = 2.5
	UIStroke14.Transparency = 0.8
	UIStroke14.Parent = DropdownSelect

	DropdownSelectReal.AnchorPoint = Vector2.new(0.5, 0.5)
	DropdownSelectReal.BackgroundColor3 = Color3.fromRGB(18, 0, 40)
	...
	function Tabs:AddTab(TabConfig)
		...
		function Sections:AddSection(Title, AlwaysOpen)
			...
			local COLOR_OPEN = Color3.fromRGB(127, 0, 255)
			...
			function Items:AddButton(ButtonConfig)
				...
				Gradient.Color = ColorSequence.new({
					...
					ColorSequenceKeypoint.new(0.5, Color3.fromRGB(140, 50, 255)),
					ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 0, 200)),
				})
			...
			function Items:AddConfigPanel()
					...
					Callback = function()
						if selectedConfigName ~= "" then
							if SaveConfigAs(selectedConfigName) then
								obsidia("Config saved: " .. selectedConfigName, 3, Color3.fromRGB(100, 255, 100))
							...
							obsidia("Please enter a config name", 3, Color3.fromRGB(255, 100, 100))
						end
					end,
					SubCallback = function()
						if selectedConfigName ~= "" then
							if LoadConfig(selectedConfigName) then
								obsidia("Config loaded: " .. selectedConfigName, 3, Color3.fromRGB(100, 255, 100))
								StatusParagraph:SetContent(
									"Current: " .. selectedConfigName .. " | Autoload: " .. (GetAutoload() or "None")
								)
							else
								obsidia("Failed to load: " .. selectedConfigName, 3, Color3.fromRGB(255, 100, 100))
							end
						else
							obsidia("Please select a config", 3, Color3.fromRGB(255, 100, 100))
						end
					end,
				})
					...
					Callback = function()
						if selectedConfigName ~= "" then
							...
							if DeleteConfig(selectedConfigName) then
								obsidia("Config deleted: " .. selectedConfigName, 3, Color3.fromRGB(255, 100, 100))
								selectedConfigName = ""
								ConfigNameInput:Set("")
								local newConfigs = GetConfigList()
							...
							obsidia("Please select a config to delete", 3, Color3.fromRGB(255, 100, 100))
						end
					end,
					SubCallback = function()
						if selectedConfigName ~= "" then
							SetAutoload(selectedConfigName)
							obsidia("Autoload set to: " .. selectedConfigName, 3, Color3.fromRGB(127, 0, 255))
							StatusParagraph:SetContent(
				...
				Items:AddButton({
					...
					SubCallback = function()
						ClearAutoload()
						obsidia("Autoload cleared", 3, Color3.fromRGB(200, 200, 100))
				...
				Items:AddButton({
					...
					Callback = function()
						ResetElements()
						obsidia("All elements reset to default values", 3, Color3.fromRGB(255, 200, 100))
					end,
				})
				...
				Items:AddButton({
					...
					Callback = function()
						if not setclipboard then
							obsidia("Clipboard not supported", 3, Color3.fromRGB(255, 100, 100))
							return
						end
						...
						setclipboard(json)
						obsidia("Config copied to clipboard", 3, Color3.fromRGB(100, 255, 100))
					end,
				})
				...
				Items:AddButton({
					...
					Callback = function()
						...
						if not json or json == "" then
							obsidia("External config JSON kosong", 3, Color3.fromRGB(255, 100, 100))
							return
						end
						...
						if ok then
							obsidia("External config loaded successfully", 3, Color3.fromRGB(100, 255, 100))
							StatusParagraph:SetContent(
								"Current: External JSON | Autoload: " .. (GetAutoload() or "None")
							)
						else
							warn(err)
							obsidia("Failed to load external JSON", 3, Color3.fromRGB(255, 100, 100))
						end
					end,
				})
